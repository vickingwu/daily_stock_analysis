...
>   - Per-industry news search is gated by `MARKET_SECTOR_NEWS_SEARCH_ENABLED` (default on) and costs one search call per industry shown (3+3 by default). Market-wide queries do not cover individual industries, so disabling it usually leaves only the attribution layer.
