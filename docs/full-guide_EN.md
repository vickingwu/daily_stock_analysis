...
| `MARKET_SECTOR_REASON_ENABLED` | Let the LLM explain the industry Top tables' driver column from actual catalysts (one LLM call per review). When set to `false` the driver column still renders, but only as sub-industry attribution | `true` |
| `MARKET_SECTOR_NEWS_SEARCH_ENABLED` | Search news per industry shown. This is the main source of catalyst evidence and costs one search call per industry (6 by default); when disabled the driver column usually falls back to sub-industry attribution | `true` |
| `MARKET_SECTOR_NEWS_MAX_RESULTS` | News items retrieved per industry (1-10) | `3` |
...
> - Industry Top tables are split into "Leading Industries Top 3" and "Lagging Industries Top 3", each with three columns: `Industry | Change | Driver`.
>   - **Industries use the SW (Shenwan) level-1 taxonomy (31 industries)**, matching how brokerages write post-market recaps. Source priority: SW level 1 `index_realtime_sw('一级行业')` → East Money level-2 boards `stock_board_industry_name_em` (86 finer boards) → Sina `stock_sector_spot`. The SW endpoint only returns index levels, so the change is computed as `(last - prev_close) / prev_close`. Degrading to East Money yields finer boards (e.g. "珠宝首饰"), which is expected.
>   - The driver cell falls back through two layers and renders `-` when neither is available; it is capped at 120 characters (240 for English reports) and escapes `|`:
>     1. **Interpretation layer** (when `MARKET_SECTOR_REASON_ENABLED=true`, an LLM is configured, and news was retrieved): one LLM call turns per-industry news plus structural material into a catalyst explanation. The prompt explicitly forbids merely restating the percentage change or advancer/decliner counts, and requires the model to abstain when it has no evidence.
>     2. **Attribution layer**: when the model abstains or is unavailable, the cell states sub-industry attribution only, e.g. `mainly dragged down by Glass fiber (-3.18%); no clear news catalyst found`. This answers "why" in the same way a brokerage recap says "mainly shipping and ports weakened", and deliberately avoids advancer/decliner counts or leading-stock percentages.
>   - Catalyst material (collected only under the SW taxonomy, all fail-open): SW level-2 sub-industry moves (parent resolved via the `上级行业` column of `sw_index_second_info()` rather than code prefixes), limit-up stocks from `stock_zt_pool_em` intersected with `index_component_sw` constituents (including consecutive-limit count and sub-industry), and top gainers/losers from the whole-market snapshot intersected with constituents. The whole-market snapshot reuses the existing 20-minute cache and degrades from East Money to Sina.
>   - Per-industry news is gated by `MARKET_SECTOR_NEWS_SEARCH_ENABLED` (default on) and tries two sources in order:
>     1. generic web search (when any of Bocha / Tavily / SerpAPI / MiniMax / Brave / SearXNG keys are configured), one call per industry;
>     2. **East Money sector news `stock_news_em` (no API key required)**, backfilling industries the first source missed. That endpoint accepts arbitrary keywords, so passing an industry name returns same-day sector coverage that usually already contains "sector + leading stock + followers + catalyst".
>   - Running without any search provider is a common deployment, in which case source 2 is the only one; market-wide news likewise falls back to the **CLS newswire `stock_info_global_cls`**.
>   - Sector news filters out pure price-restatement listings (limit-up roundups, "low-priced stock" tables, intraday board reports, industry gain/loss rankings, AI-generated boilerplate) so the model is not nudged into restating the change.
>   - `market_review_payload.sectors.{top,bottom}[]` gains optional `reason`, `reason_source` (`llm` / `sub_sector_attribution`), `taxonomy` (`sw_l1` / `em_l2` / `sina`) and `code` fields. `name` / `change_pct` are unchanged — fields are only appended.
